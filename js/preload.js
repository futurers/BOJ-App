...
const data = fs.readFileSync(`${__dirname}/lang.json`, 'utf8');
...
const TIO = {
  ...
  run:
    async function run(code, input, lang) {
      ...
      const { length } = encoder.encode(code);
      const iLength = encoder.encode(input).length;
      //  Vlang\u00001\u0000{language}\u0000F.code.tio\u0000{# of bytes in code}\u0000{code}F.input.tio\u0000{length of input}\u0000{input}letgs\u0000{number of ARGV}{ARGV}\u0000R
      let rBody = `Vlang\x001\x00${lang}\x00F.code.tio\x00${length}\x00${code}F.input.tio\x00${iLength}\x00${input}letgs\x000\x00R`;
      rBody = encode(rBody);
  ...
  makeLink:
    function makeLink(languageId, header = '', code = '', footer = '', input = '', args = [], options = [], fullLink = true) {
      ...
      const saveTextArea = function (textArea) {
  ...
  parseLink:
    function parseLink(link) {
      ...
      const stateString = byteArrayToByteString(inflate(b64ToArr(link)));

      let fields = stateString.split(startOfExtraFields);
      fields = fields.map((n) => n.split(fieldSeparator));

      let [languageId, header, code, footer, input, ...args] = fields[0];
...
window.addEventListener('DOMContentLoaded', (event) => {
    localStorage.setItem("be_count", 0);
    ...
    if(localStorage.getItem('boj-extended-activated') == "true")
    {
        ...
        if(localStorage.getItem("be_count") == 0)
        {
            be_fnc();
            localStorage.setItem("be_count", 1);
    ...
    if (document.URL.startsWith('https://www.acmicpc.net/submit/')) {
        document.getElementsByClassName("form-horizontal submit-form")[0].style.width = "50%";
        document.getElementsByClassName("form-horizontal submit-form")[0].style.float= "right";
...
window.onload = () => {
    ...
    if (document.URL.startsWith('https://www.acmicpc.net/submit/')) {
        ...
        if (lang_json.hasOwnProperty(document.getElementsByClassName('chosen-single')[0].innerText)) {
            ...
            new Notify({
                status: 'error', title: '실행 결과:', text: '<b>지원되지 않는 언어입니다</b>', effect: 'fade', speed: 300, customClass: null, customIcon: 'Boj', showIcon: true, showCloseButton: true, autoclose: true, autotimeout: 8000, gap: 20, distance: 20, type: 1, position: 'right top',
            });
            return;
            }
            line = document.getElementsByClassName(' CodeMirror-line ');
            let code = ''; //* [@id="submit_form"]/div[3]/div/div/div[6]/div[1]/div/div/div/div[5]/div[3]/pre/span
            for (let i = 0; i < line.length; i++) {
            code += `${line[i].innerText.replace(/[\u200B-\u200D\uFEFF]/g, '')}\n`;
            }
            TIO.run(beautify(code, { format: 'js' }), document.getElementById('check_field').value, lang_json[document.getElementsByClassName('chosen-single')[0].innerText]).then((n) => new Notify({
            status: 'success',
            title: '실행 결과:',
            text: `<b><p style='font-size: 14px; color: black;'>stdout: ${n[0]} </p></b>
                    stderr+timing: ${n[1]} `,
            effect: 'fade',
            speed: 300, 
            customClass: null,
...
window.addEventListener('onpushchange', function(){
    localStorage.setItem("be_count", 0);
})
